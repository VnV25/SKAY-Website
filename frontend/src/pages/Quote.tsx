...
export function Quote() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      description: `${formData.color ? `Color: ${formData.color}` : ''} ${formData.size ? `Size: ${formData.size}` : ''} ${formData.description || ''}`.trim(),
    };
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unable to submit quote request at this time.';
      console.error('Failed to submit quote request:', errorMessage);
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {error && (
            <div className="mb-6 p-4 bg-red-100 border border-red-300 rounded-md">
              ...
              <p className="text-red-600 text-sm mt-1">Please check your form and try again.</p>
            </div>
          )}
